...
const Messages: React.FC = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    // Simulate AI Response from Shelter
    try {
      const prompt = `You are a shelter staff member at "${selectedChat.shelterName}". 
      The user Jane is asking about the pet "${selectedChat.petName}" (${selectedChat.petBreed}).
      Latest message from Jane: "${newMessage}"
      Keep the response warm, professional, and under 2 sentences. 
      Jane has a bio: "Active outdoor enthusiast. I hike every weekend and love high-energy dogs."`;
      ...
      const shelterMsg: ChatMessage = {
        ...
        text: response.text || "Thanks for your message! We'll get back to you soon.",
        sender: 'shelter',
        timestamp: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
      };
        ...
        {tabs.map((tab) => (
          <button
            ...
            className={`pb-3 text-sm font-bold tracking-wide transition-all relative ${
              activeTab === tab ? 'text-slate-900' : 'text-gray-400'
            }`}
          >
            {tab}
            {activeTab === tab && (
